...
import { Apollo } from 'apollo-angular'; // THÊM IMPORT NÀY
...
@Component({
  selector: 'app-root',
  standalone: true,
  imports: [MatIconModule, CommonModule, SidebarComponent, RouterModule, MatTabsModule, MatButtonModule, MatRippleModule,KhoThanhPhamModule],
...
export class AppComponent implements OnInit, OnDestroy {
  ...
  constructor(public sidebarservice: SidebarService,
    ...
    private apollo: Apollo // INJECT APOLLO VÀO ĐÂY
  ) {
  ...
  ngOnInit(): void {
    ...
    this.routerSubscription = this.router.events.pipe(
      ...
      mergeMap(route => route.data.pipe(map(data => ({ path: this.router.url.split('?')[0], data }))))
    ).subscribe(eventData => {
      console.log('[AppComponent] Processing path for tab:', eventData.path); // DEBUG LOG
      const path = eventData.path;
      const label = eventData.data['tabLabel'] || this.generateLabelFromPath(path);

      if (path && label && path !== '/') {
        this.addTab({ path, label, isClosable: eventData.data['isClosable'] !== undefined ? eventData.data['isClosable'] : true });
      } else if (path === '/' && this.links.length === 0 && eventData.data['tabLabel']) {
        this.addTab({ path, label, isClosable: eventData.data['isClosable'] !== undefined ? eventData.data['isClosable'] : true });
      }
    });
  }
  ...
  private generateLabelFromPath(path: string): string {
    ...
    if (path === '/home' || path === '/') return 'Trang chủ';
  ...
  addTab(newLink: TabLink): void {
    ...
    this.activeLink = this.links.find(link => link.path === newLink.path) || null;
  }
  ...
  removeTab(tabToRemove: TabLink, event: MouseEvent): void {
    ...
    const index = this.links.findIndex(link => link.path === tabToRemove.path);
    if (index > -1) {
      ...
      if (this.activeLink && this.activeLink.path === tabToRemove.path) {
        if (this.links.length > 0) {
          ...
          const newActiveIndex = Math.max(0, Math.min(index, this.links.length - 1));
